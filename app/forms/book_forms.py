...
class ManualBookAddForm(FlaskForm):
    ...
    published_date = StringField('Published Date', validators=[Optional(), Length(max=500)])
    categories = TextAreaField('Categories (comma separated)', validators=[Optional()])
    ...
    pdf_upload = FileField('Upload PDF (optional)', validators=[Optional(), FileAllowed(['pdf'], 'PDF only!')])
