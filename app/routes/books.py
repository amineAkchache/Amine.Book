...
import os
import requests
from flask import send_from_directory, Blueprint, current_app, render_template, url_for, flash, redirect, request, jsonify
...
def add_manual():
    ...
    if form.validate_on_submit():
        ...
        if pdf_file:
            ...
            form.pdf_upload.data.save(pdf_path)
...
def add_from_api(google_id):
    ...
    pdf_path = os.path.join(current_app.config.get('UPLOAD_FOLDER', 'uploads'), 
           secure_filename(f"{google_id}.pdf"))
    filename = f"{google_id}.pdf"
...
def add_book():
    ...
    if form.validate_on_submit():
        ...
        pdf_file = form.pdf_upload.data
        if pdf_file:
            # Call the model's save_pdf_file method to handle saving the file
            # pdf_path = Book.save_pdf_file(pdf_file)
            pdf_path = f"PDF_UPLOAD_PATH=./uploads{pdf_file}"
        else: 
        ...
        new_book = Book(
            ...
            categories=form.categories.data,
            page_count=form.page_count.data,
            google_books_id=None,  # Set to None if not using Google Books
            date_added=datetime.utcnow(),
            pdf_file=pdf_path,  # Save the file path in the database
            user_id=current_user.id
        )
